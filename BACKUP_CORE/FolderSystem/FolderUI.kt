package com.riprog.launcher.backup.folder
...
class FolderUI(
    ...
    fun createFolderView(item: HomeItem, isOnGlass: Boolean, cellWidth: Int, cellHeight: Int): View {
        ...
        if (item.spanX <= 1.0f && item.spanY <= 1.0f) {
            val baseSize = context.resources.getDimensionPixelSize(
                context.resources.getIdentifier("grid_icon_size", "dimen", context.packageName)
            )
        ...
        // refreshFolderPreview(item, grid) logic should be called here or externally
