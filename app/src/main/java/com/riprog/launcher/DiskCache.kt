...
class DiskCache(context: Context) {
    ...
    fun saveIcon(type: String, key: String, bitmap: Bitmap) {
    ...
    fun removeIcon(type: String, key: String) {
        val file = File(File(rootDir, type), "$key.webp")
    ...
    fun loadIcon(type: String, key: String): Bitmap? {
        val file = File(File(rootDir, type), "$key.webp")
    ...
    fun saveData(type: String, key: String, data: String) {
    ...
    fun invalidateData(type: String, key: String) {
        var file = File(File(rootDir, type), key + (if (TYPE_LAYOUT == type) ".json" else ".dat"))
        if (!file.exists() && "data" == type) {
            file = File(File(rootDir, type), "$key.json")
        }
    ...
    fun getDataLastModified(type: String, key: String): Long {
        val file = File(File(rootDir, type), key + (if (TYPE_LAYOUT == type) ".json" else ".dat"))
    ...
    private fun performSmartCleanup() {
        ...
        if (totalSize > MAX_CACHE_SIZE) {
            allFiles.sortWith { f1: File, f2: File ->
                val name1 = f1.name
                val name2 = f2.name
