...
class SettingsManager(private val context: Context) {
    ...
    var columns: Int
        ...
        set(columns) = prefs.edit().putInt(KEY_COLUMNS, columns).apply()
    var widgetId: Int
        get() = prefs.getInt(KEY_WIDGET_ID, -1)
        set(widgetId) = prefs.edit().putInt(KEY_WIDGET_ID, widgetId).apply()
    fun isFreeformHome(): Boolean {
        return prefs.getBoolean(KEY_FREEFORM_HOME, false)
    }
    fun setFreeformHome(freeform: Boolean) {
        prefs.edit().putBoolean(KEY_FREEFORM_HOME, freeform).apply()
    }
    var iconScale: Float
        get() = prefs.getFloat(KEY_ICON_SCALE, 1.0f)
        set(scale) = prefs.edit().putFloat(KEY_ICON_SCALE, scale).apply()
    fun isHideLabels(): Boolean {
        return prefs.getBoolean(KEY_HIDE_LABELS, false)
    }
    fun setHideLabels(hide: Boolean) {
        prefs.edit().putBoolean(KEY_HIDE_LABELS, hide).apply()
    }
    fun isLiquidGlass(): Boolean {
    ...
    var themeMode: String?
        ...
        set(mode) = prefs.edit().putString(KEY_THEME_MODE, mode).apply()
    ...
    var drawerOpenCount: Int
        ...
        set(value) = prefs.edit().putInt(KEY_DRAWER_OPEN_COUNT, value).apply()
    fun incrementDrawerOpenCount() {
        drawerOpenCount += 1
    }
    var lastDefaultPromptTimestamp: Long
        get() = prefs.getLong(KEY_DEFAULT_PROMPT_TIMESTAMP, 0)
        set(ts) = prefs.edit().putLong(KEY_DEFAULT_PROMPT_TIMESTAMP, ts).apply()
    var defaultPromptCount: Int
        get() = prefs.getInt(KEY_DEFAULT_PROMPT_COUNT, 0)
        set(value) = prefs.edit().putInt(KEY_DEFAULT_PROMPT_COUNT, value).apply()
