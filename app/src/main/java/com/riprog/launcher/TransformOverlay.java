...
public class TransformOverlay extends FrameLayout {
    ...
    private void setupButtons() {
        ...
        addButton(container, R.string.action_remove, adaptiveColor, v -> { if (onSaveListener != null) onSaveListener.onRemove(); });
        addButton(container, R.string.action_reset, adaptiveColor, v -> reset());
        addButton(container, R.string.action_save, adaptiveColor, v -> { if (onSaveListener != null) onSaveListener.onSave(); });

        if (item.type == HomeItem.Type.APP) {
            addButton(container, R.string.action_app_info, adaptiveColor, v -> { if (onSaveListener != null) onSaveListener.onAppInfo(); });
            addButton(container, R.string.drag_uninstall, adaptiveColor, v -> { if (onSaveListener != null) onSaveListener.onUninstall(); });
        }
    ...
    private void addButton(LinearLayout parent, int textRes, int color, OnClickListener listener) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (activeHandle != -1) {
                    if (!hasPassedThreshold) {
                        float threshold = dpToPx(MOVE_THRESHOLD_DP);
    ...
    private int findHandle(float tx, float ty) {
        ...
        if ((isFreeform || item.type == HomeItem.Type.WIDGET || item.type == HomeItem.Type.FOLDER) &&
            dist(rx, ry, (left + right) / 2f, top - rotationHandleDist) < hs) return HANDLE_ROTATE;
    ...
    private void handleInteraction(float tx, float ty) {
        ...
        if (activeHandle == ACTION_MOVE) {
            ...
            if (!isFreeform) {
                int cellWidth = gridManager.getCellWidth(getWidth());
                int cellHeight = gridManager.getCellHeight(getHeight());
            ...
            float rotAngle = (float) Math.toRadians(-targetView.getRotation());
            ...
            switch (activeHandle) {
                case HANDLE_TOP:
                    if (halfContentH > 0) newScaleY = Math.max(0.2f, Math.abs(ry) / halfContentH);
                    break;
                case HANDLE_BOTTOM:
                    if (halfContentH > 0) newScaleY = Math.max(0.2f, Math.abs(ry) / halfContentH);
                    break;
                case HANDLE_LEFT:
                    if (halfContentW > 0) newScaleX = Math.max(0.2f, Math.abs(rx) / halfContentW);
                    break;
                case HANDLE_RIGHT:
                    if (halfContentW > 0) newScaleX = Math.max(0.2f, Math.abs(rx) / halfContentW);
                    break;
