...
import com.riprog.launcher.features.home.TransformOverlay
...
class FreeformController(
    ...
    private fun snapToGrid(v: View, isResize: Boolean = false): Boolean {
        ...
        if (otherView != null) {
            val cBounds = WidgetUtils.getVisualBounds(otherView)
    ...
    private fun saveTransform() {
        ...
        if (cellWidth > 0 && cellHeight > 0) {
            ...
            if (preferences.isFreeformHome || item.type == HomeItem.Type.FOLDER) {
                ...
                val vBounds = WidgetUtils.getVisualBounds(transformingView!!)
                val sX: Int
                val sY: Int
